...
LineRectTempInfo_t* temp_command(StreamFrameInfo_t* handle, int x1, int y1, int x2, int y2){
    IrcmdHandle_t* ircmd_handle = handle->ircmd_handle;
    LineRectTempInfo_t* rect_temp_info = NULL;
    
    if (ircmd_handle == NULL)
    {
        printf("ircmd_handle is NULL\n");
        return rect_temp_info;
    }
    int param;
    int ret;
    IrcmdRect_t rect_pos;
    rect_temp_info = new LineRectTempInfo_t();
    //memset(&rect_temp_info, 0, sizeof(rect_temp_info));
    
    //scanf("%hd %hd", &(rect_pos.start_point.x), &(rect_pos.start_point.y));
    //scanf("%hd %hd", &(rect_pos.end_point.x), &(rect_pos.end_point.y));
    rect_pos.start_point.x = x1;
    rect_pos.start_point.y = y1;
    rect_pos.end_point.x = x2;
    rect_pos.end_point.y = y2;
...
void* cmd_function(void* threadarg)
{
    ...
    while (isRUNNING)
    {
        ...
        temp_command((StreamFrameInfo_t*)threadarg, 0, 0, 255, 191);
    }

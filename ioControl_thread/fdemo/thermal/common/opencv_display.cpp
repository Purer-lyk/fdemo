...
cv::Mat display_one_frame(uint8_t* frame, uint16_t width, uint16_t height, const char* title)
{
...
void* opencv_display_function(void* threadarg)
{
    ...
    while (isRUNNING)
    {
        ...
        display_one_frame(bgr_image_frame, stream_frame_info->width, stream_frame_info->height, "image").copyTo(bgrImage);
        ...
        /*if ((stream_frame_info->frame_output_format == YUYV_AND_TEMP) || (stream_frame_info->frame_output_format == NV12_AND_TEMP))
        {
            ...
            rgb_to_bgr(rgb_image_frame, (stream_frame_info->width * stream_frame_info->height), bgr_image_frame);
            display_one_frame(bgr_image_frame, stream_frame_info->width, stream_frame_info->height, "temp").copyTo(tempImage);
        }*/
